...
module game {
  ...
  function isMultiPlayerGame():boolean{
    return (currentUpdateUI.playMode==0 || currentUpdateUI.playMode==1) && currentUpdateUI.matchType=="pingPongMultiplayer";
  }
  export function updateUI(params: IUpdateUI): void {
    ...
    if(state!=null && state.delta!=null){
      ...
      if(!replayForMultiplayer){
        ...
        animationEndedTimeout = $timeout(function(){animationEndedCallback(sourceCopy)},time_out*state.deltaArray.length+time_out+600);
      }
    }
  ...
  function animationEndedCallback(sourceCopy:string[][][]) {
    ...
    if(state.nextMoveType==null){
      ...
      if(currentUpdateUI.matchType == 'speedMultiplayer'){
